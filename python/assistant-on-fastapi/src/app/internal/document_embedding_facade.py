...
class DocumentEmbeddingApiFacade:
    ...
    async def create_document_embedding(self, document_embeddings: list[CreateDocumentEmbeddingRequestDto]):
        docs = document_embedding_mapper.to_dict_of_document_embedding_list(document_embeddings)

        async with self._engine.connect() as connection, AsyncSession(connection).begin() as session_tx:
    ...
    async def search_vector_similarity(self, search_similarity_request: SearchVectorSimilarityRequestDto):
        async with self._engine.connect() as connection, AsyncSession(connection).begin() as session_tx:
            session_tx: AsyncSessionTransaction
            repository = DocumentEmbeddingRepository(session_tx.session)
            result_cursor = await repository.search_vector_similarity(search_similarity_request)
            results = [
                document_embedding_mapper.to_search_vector_similarity_response_dto(row)
