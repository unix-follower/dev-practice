...
class FoodPredictApiFacade(ABC):
    @abstractmethod
    async def predict(self, food_predict_request_dto: FoodPredictRequestDto) -> FoodPredictResponseDto:
        pass
...
class FoodPredictFacade(FoodPredictApiFacade):
    ...
    async def predict(self, food_predict_request_dto: FoodPredictRequestDto):
        if food_predict_request_dto.model not in self._available_ml_models:
            raise AppException("Unsupported model", error_code=ErrorCode.UNSUPPORTED_ML_MODEL)
        ...
        input_data = np.array([feature.name, feature.description, feature.category]).reshape(1, 3)
