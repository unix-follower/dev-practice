...
from app.config.dependency_injection import get_document_embedding_facade
...
async def create_document_embedding(
    ...
    document_embedding_api_facade: Annotated[
        DocumentEmbeddingApiFacade, Depends(get_document_embedding_facade)
    ],
):
...
async def search_vector_similarity(
    ...
    return await document_embedding_api_facade.search_vector_similarity(
        similarity_request
    )
