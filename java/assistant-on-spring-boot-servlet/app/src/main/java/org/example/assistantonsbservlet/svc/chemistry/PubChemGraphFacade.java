...
@Component
public class PubChemGraphFacade implements PubChemGraphApiFacade {
    private final ApplicationContext appContext;
    private final ConversionService appConversionService;
    private EntityManagerFactory emf;

    public PubChemGraphFacade(ApplicationContext appContext) {
    ...
    @Override
    public ChemistryGraphResponse getCompoundDataByName(int page, int pageSize, String compoundName) {
        final var em = emf.createEntityManager();
        final var tx = em.getTransaction();
        try (em) {
            tx.begin();
